/// Reminder/nudge texts for work-hour focus, protection levels, and after-work anti-doomscroll.
///
/// These are local fallback defaults. The server may override them via
/// [RemindersStore.setServerData] — fetched once on app start and cached.
library;
...
class RemindersStore {
  static final RemindersData _fallback = RemindersData(
    focus: _workFocusReminders,
  ...
  static RemindersData get current => _server ?? _fallback;

  static void applyServer(Iterable<Map<String, dynamic>> rows) {
    final focus = <String>[];
    final anchor = <String>[];
...
// ── Work-focus reminders (Green protection, 30-min intervals) ──────────
const _workFocusReminders = [
...
// ── Body-anchor reminders (Yellow protection, 20-min intervals) ───────
const _bodyAnchorReminders = [
...
// ── Physical-interrupt reminders (Red protection, 15-min intervals) ────
const _physicalInterruptReminders = [
...
// ── Anti-doomscroll nudges (after-work hours) ──────────────────────────
const _antiDoomscrollNudges = [
  '🧠 执行意图：如果手伸向短视频→立即站起来喝一口水，用身体动作打断自动化反应。预先决定比临场抵抗有效2倍。',
  '🏄 冲动冲浪：刷短视频的渴望像海浪，会在20分钟内自然消退。不用对抗它——观察它上升、停留、退去。你现在正处于波浪的哪一段？',
  '🧘 分离觉察："我有一个想刷短视频的冲动"——看见它，不评判，不追随。念头只是大脑的天气预报，你不是必须带伞。',
  '🚪 环境重构：给手机设灰度模式、把短视频APP移到第二屏的文件夹里、在沙发上放一本摊开的书。让好选择比坏选择更省力。',
  '⏳ 5分钟延迟：想刷的时候对自己说"先做5分钟正事，想刷再刷"。5分钟后多巴胺峰值已过，你会发现自己根本没开始刷。',
  '🔮 未来投射：想象今晚9点——你为自己完成了计划而满足，还是为刷了2小时而自责？用具体画面判断。你选哪个版本的今晚？',
  '👁 注意力外移：当刷屏冲动出现时，立即关注一件外部事物——窗外树叶的纹理、键盘的触感、呼吸时胸腔的起伏。把注意力从"内部渴求"拉回"外部现实"。',
