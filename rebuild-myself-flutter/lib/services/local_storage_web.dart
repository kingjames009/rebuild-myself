...
class LocalStorage {
  ...
  Future<List<Map<String, dynamic>>> loadTable(String table) async {
    ...
    try {
      final data = html.window.localStorage[table];
  ...
  Future<void> _saveTable(String table) async {
    html.window.localStorage[table] = json.encode(_cache[table] ?? []);
  }
  ...
  bool _matchWhere(Map<String, dynamic> row, String where, List<dynamic> whereArgs) {
    // Split by AND to support compound conditions like "planDate = ? AND timePeriod = ?"
