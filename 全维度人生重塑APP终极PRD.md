...
5. **上班时段正念干预**：上班时间自动填入正念专注内容，应对胡思乱想和情绪波动
...
4. 上班时段（上午+下午）：**MCT 元认知疗法协议每日循环**（5项临床练习：ATT注意训练、分离觉察、反刍中断3步法、具体化切换、行为实验）。若用户有日间习惯则用习惯库内容替代
...
#### 每小时实际状况记录（2026-05-13 新增）

- **核心理念**：不是"完成勾选"（做没做到），而是记录"这个时段实际发生了什么"，为 AI 日复盘提供真实素材
- **入口**：首页深色「当前时段」聚焦卡片右上角 + 按钮；首页下方「今日规划」列表中每项均可点击
- **存储**：写入 `daily_model_plan.actual_note`，通过 `PUT /api/plan/note` 同步到 MySQL
...
| PUT | `/api/plan/note` | 按日期+时段更新实际状况记录 |
...
> **2026-05-13 新增字段**：`is_completed`、`actual_note`、`completed_at`。`actual_note` 用于每小时实际状况记录，非完成标记，而是记录"这个时段实际发生了什么"，供 AI 日复盘分析使用。
...
- **上班时段内容**：默认使用 MCT 元认知疗法 5 项练习循环，用户可在日间习惯库中添加自定义内容替代
